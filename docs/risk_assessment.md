...
| R-09 | Unreviewed or malicious changes reach a project's main branch | Active project repositories | External contributors use the fork-and-pull request flow and have no direct write access. Write access requires an explicit grant. This repository has a `CODEOWNERS` file that nominates reviewers, but required approval and branch protection are not confirmed. | TL review | TL review: confirm required reviews and branch protection on active project repositories. This is not covered by current documentation. |
